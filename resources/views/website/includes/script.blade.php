...
<script>
    $(document).ready(function() {
        ...
        $("#xzoom-default").xzoom({
            position: 'right', // Can be 'right', 'left', 'inside', 'lens'
            lens: true,
            Xoffset: 15
        });
    });
...
<script>
        ...
        if (diff < 0) {
    // Show the "Event Ended" alert
    document.querySelector('.alert').style.display = 'block';
    
    // Target the specific timer box instead of the whole page's containers
    const timerBox = document.querySelector('.box-head');
    if (timerBox) {
        timerBox.style.display = 'none';
    }
    
    // Stop the interval to save performance
    return; 
}
    </script>
