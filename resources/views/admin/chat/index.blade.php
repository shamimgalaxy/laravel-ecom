...
.chat-sidebar        { width:300px; border-right:1px solid #e2e8f0; display:flex; flex-direction:column; flex-shrink:0; }
...
.conversation-item.active { background:#f0f6ff; }
...
.chat-main-header { padding:14px 20px; border-bottom:1px solid #e2e8f0; background:#f8fafc; display:flex; align-items:center; gap:10px; flex-shrink:0; }
...
.chat-footer   { padding:12px 16px; border-top:1px solid #e2e8f0; background:#fff; display:flex; gap:10px; align-items:center; flex-shrink:0; }
...
<div class="chat-container">
    ...
    <div class="chat-sidebar">
        ...
        <div class="conversation-list" id="conversationList">
            ...
            @empty
            <div style="padding:40px;text-align:center;color:#94a3b8;"><p>No conversations yet</p></div>
            @endforelse
        </div>
    </div>

    {{-- ── Main area — footer lives INSIDE here from the start ── --}}
    <div class="chat-main" id="chatMain">
        ...
        <div id="chatMainHeader" class="chat-main-header" style="display:none;"></div>

        <div class="chat-messages" id="chatMessages" style="display:none;"></div>

        {{-- Footer always inside chatMain --}}
        <div class="chat-footer" id="chatFooter" style="display:none;">
            <input
                class="chat-input"
                id="adminChatInput"
                name="admin_chat_message"
                type="text"
                placeholder="Type a reply..."
                autocomplete="off"
                aria-label="Type a reply"
            />
            <button class="chat-send-btn" onclick="sendAdminMessage()" aria-label="Send reply">
...
(function () {
    ...
    pusher.connection.bind('error',     function (err) { console.error('Pusher error:', err); });

    var chatChannel = pusher.subscribe('chat');
    chatChannel.bind('.message.sent', function (data) {
        if (data.sender === 'customer') {
            if (String(data.customer_id) === String(currentCustomerId)) {
    ...
    // ── Update header ─────────────────────────────────────────
    function updateChatHeader(customerName, customerStatus, isActive) {
        var header = document.getElementById('chatMainHeader');
        if (!header) return;
        header.innerHTML = headerInnerHTML(customerName, customerStatus, isActive);
    }

    function headerInnerHTML(customerName, customerStatus, isActive) {
        return '<div style="position:relative;">' +
                    '<div class="chat-main-avatar">' + escapeHtml(customerName.substring(0, 2).toUpperCase()) + '</div>' +
                    (isActive ? '<span style="position:absolute;bottom:1px;right:1px;width:8px;height:8px;border-radius:50%;background:#22c55e;border:2px solid #f8fafc;"></span>' : '') +
                '</div>' +
                '<div>' +
                    '<div class="chat-main-name">' + escapeHtml(customerName) + '</div>' +
                    '<div class="chat-main-status" style="color:' + (isActive ? '#22c55e' : '#94a3b8') + ';">' + escapeHtml(customerStatus) + '</div>' +
                '</div>';
    }
    ...
    function loadConversation(customerId, el) {
        ...
        // Show UI panels
        showChatUI();
        updateChatHeader(customerName, customerStatus, isActive);

        // Set loading state
        var container = document.getElementById('chatMessages');
        container.innerHTML = '<div style="text-align:center;color:#94a3b8;font-size:12px;padding:20px;">Loading messages...</div>';

        // Fetch live customer status
        fetch('/admin/chat/customer/' + customerId, {
            headers: { 'X-CSRF-TOKEN': CSRF_TOKEN, 'Accept': 'application/json' }
        })
        .then(function (r) {
            var ct = r.headers.get('content-type') || '';
            if (!r.ok || !ct.includes('application/json')) throw new Error('Non-JSON');
            return r.json();
        })
        .then(function (data) {
            el.dataset.customerStatus = data.status;
            el.dataset.isActive       = data.is_active ? 'true' : 'false';
            updateChatHeader(data.name, data.status, data.is_active);
        })
        .catch(function () {});
    ...
    function loadMessages(customerId) {
        ...
        .then(function (data) {
            ...
            if (!data.messages || data.messages.length === 0) {
                container.innerHTML = '<div style="text-align:center;color:#94a3b8;font-size:12px;padding:20px;">No messages yet</div>';
                return;
            }
        ...
        .catch(function (err) {
            ...
            var container = document.getElementById('chatMessages');
            if (container) {
                container.innerHTML = '<div style="text-align:center;color:#ef4444;font-size:12px;padding:20px;">Failed to load messages.</div>';
            }
        });
    }
    ...
    function sendAdminMessage() {
        ...
        fetch('/admin/chat/send', {
            ...
            body: JSON.stringify({ message: text, customer_id: currentCustomerId })
        })
        .then(function (r) {
            var ct = r.headers.get('content-type') || '';
            if (!r.ok || !ct.includes('application/json')) throw new Error('Non-JSON');
  ...
  function appendMessage(text, fromType, isoTime) {
    ...
    function updateConversationPreview(customerId, message) {
        var items = document.querySelectorAll('[data-customer-id="' + customerId + '"] .conv-preview');
        if (items.length > 0) items[items.length - 1].textContent = message.substring(0, 30);
    }
